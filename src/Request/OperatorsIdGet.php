 ...
 */
class OperatorsIdGet extends Request
{
    /**
    ...
    {
        $rules = array('fields' => array(new OneOf(array('hardwareConfigurations(hardware(name))', 'telecommunicationStandard(name)', 'currentRuntimes(hardware(name),runtime(name))', 'upgradeStatus(runtime(name))'))));
        return $rules;
    }
}